...
def _keypair():
    ...
    pub_der = k.public_key().public_bytes(serialization.Encoding.DER, serialization.PublicFormat.SubjectPublicKeyInfo)
    pub_pem = (
        k.public_key()
...
def status_body():
    ...
    core = {
        ...
        "escapes": s["escapes"],
        "paywall": s["paywall"],
        "timer_remaining_ms": s["timer_ms"],
        "task_reps": s["task_reps"],
        "task_done": s["task_done"],
        "offer": s["offer"],
        "offer_status": s["offer_status"],
        "sub_tier": s["sub_tier"],
    ...
    orders = {
        "lock_active": 1 if s["locked"] else 0,
        "message": "",
        "task_text": "",
        "task_reps": s["task_reps"],
        "task_done": s["task_done"],
...
if __name__ == "__main__":
    ...
    threading.Thread(target=lambda: HTTPServer(("127.0.0.1", PORT + 1), Control).serve_forever(), daemon=True).start()
