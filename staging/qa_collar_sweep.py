...
from qa_collar_driver import post, settings
...
check(
    "lock: basic + 15m timer",
    "/api/lock",
    {"timer": "15", "mode": "basic", "message": "QA sweep"},
    {
        "focus_lock_active": "1",
        "focus_lock_mode": "basic",
        "focus_lock_unlock_at": nonzero,
        "focus_lock_message": "QA sweep",
    },
)

for mode in ("negotiation", "gratitude", "exercise", "love_letter", "compliment"):
    check(f"lock mode: {mode}", "/api/lock", {"timer": "5", "mode": mode}, {"focus_lock_mode": mode})

check(
    "lock mode: random resolves to a real mode",
    "/api/lock",
    {"timer": "5", "mode": "random"},
    {"focus_lock_mode": lambda v: v in ("basic", "negotiation", "gratitude", "exercise", "love_letter")},
)
...
check(
    "lock mode: photo_task", "/api/photo-task", {"task": "selfie", "hint": "smile"}, {"focus_lock_photo_task": nonzero}
)

# ── modifiers / toggles ────────────────────────────────────────────────────
check(
    "modifiers: vibrate+penalty+shame+dim+mute",
    "/api/lock",
    {"timer": "5", "mode": "basic", "vibrate": "1", "penalty": "1", "shame": "1", "dim": "1", "mute": "1"},
    {
        "focus_lock_vibrate": "1",
        "focus_lock_penalty": "1",
        "focus_lock_shame": "1",
        "focus_lock_dim": "1",
        "focus_lock_mute": "1",
    },
...
check(
    "word_min / exercise mode params",
    "/api/lock",
    {"timer": "5", "mode": "love_letter", "word_min": "120", "exercise": "50 squats"},
    {"focus_lock_word_min": "120", "focus_lock_exercise": "50 squats"},
)

# ── money ──────────────────────────────────────────────────────────────────
check("paywall: add $25", "/api/add-paywall", {"amount": "25"}, {"focus_lock_paywall": "25"})
check("paywall: add stacks to $40", "/api/add-paywall", {"amount": "15"}, {"focus_lock_paywall": "40"})
check(
    "paywall: lock without amount must NOT clobber the ledger",
    "/api/lock",
    {"timer": "5", "mode": "basic"},
...
check("subscription: subscribe silver", "/api/subscribe", {"tier": "silver"}, {"focus_lock_sub_tier": "silver"})
...
check(
    "subscription: local unsubscribe is refused (server-authoritative)",
    "/api/unsubscribe",
    None,
    {},
    expect_refusal="server-authoritative",
)

# ── messaging ──────────────────────────────────────────────────────────────
check("message to bunny", "/api/message", {"message": "kneel"}, {"focus_lock_message": "kneel"})
check("pinned message", "/api/pin-message", {"message": "good bunny"}, {"focus_lock_pinned_message": "good bunny"})

# ── negotiation / offers ───────────────────────────────────────────────────
check("offer: bunny proposes", "/api/offer", {"offer": "10 min early?"}, {"focus_lock_offer": lambda v: "10 min" in v})
check(
    "offer: Lion responds",
    "/api/offer-respond",
...
check(
    ...
    {"focus_lock_geofence_lat": lambda v: v.startswith("43.65"), "focus_lock_geofence_radius_m": "150"},
)
check(
    "geofence: clear", "/api/clear-geofence", None, {"focus_lock_geofence_radius_m": lambda v: v in ("", "null", "0")}
)
check("check-in deadline (hour of day)", "/api/set-checkin", {"deadline": "22"}, {"focus_lock_checkin_deadline": "22"})
check("check-in rejects an out-of-range hour", "/api/set-checkin", {"deadline": "99"}, {}, expect_refusal="0-23")
...
check("free unlock refused below Gold", "/api/free-unlock", None, {}, expect_refusal="Gold")
check("subscription: upgrade to gold", "/api/subscribe", {"tier": "gold"}, {"focus_lock_sub_tier": "gold"})
check(
    "free unlock granted at Gold (and releases the lock)",
    "/api/free-unlock",
    None,
    {"focus_lock_free_unlocks": "1", "focus_lock_active": "0"},
)
check("free unlock is once per month", "/api/free-unlock", None, {}, expect_refusal="already used")
check(
    "notification prefs",
    "/api/set-notif-prefs",
...
check("re-lock before testing unlock", "/api/lock", {"timer": "10", "mode": "basic"}, {"focus_lock_active": "1"})
check(
    "unlock", "/api/unlock", None, {"focus_lock_active": "0", "focus_lock_unlock_at": lambda v: v in ("0", "", "null")}
)

# ── hardware-blocked ───────────────────────────────────────────────────────
check("Lovense toy control", "/api/lovense", None, {}, blocked="no Lovense device paired to this bunny phone")
check("TTS speak", "/api/speak", None, {}, blocked="audible-only; no way to assert from adb")
check("play audio", "/api/play-audio", None, {}, blocked="audible-only; no way to assert from adb")
check("screen power control", "/api/power", None, {}, blocked="would blank the screen mid-sweep; skipped deliberately")
check("Release Forever", "/api/release-forever", None, {}, blocked="destructive teardown + self-uninstall; never swept")
check(
    "entrapment mode", "/api/entrap", None, {}, blocked="raises the release bar on a real device; not swept unattended"
)
