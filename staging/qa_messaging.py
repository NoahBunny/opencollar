...
def call(op, body):
    req = urllib.request.Request(
        f"{RELAY}/api/mesh/{MESH}/messages/{op}",
...
def fetch(since=0):
    ...
    return call("fetch", {"node_id": NODE, "from": "bunny", "since": since, "ts": ts, "signature": sign(payload)})
...
record("thread round-trip: message is retrievable", code == 200 and found, f"HTTP {code}, {len(msgs)} msg(s) in thread")
...
record("bunny CANNOT forge a message from the Lion", code == 403, f"HTTP {code} {resp.get('error', '')[:60]}")

ts = int(time.time() * 1000)
payload = f"{MESH}|{NODE}|lion|whatever|0|0|{ts}"
code, resp = call(
    "edit",
    {"node_id": NODE, "from": "lion", "message_id": "1", "text": "rewritten", "ts": ts, "signature": sign(payload)},
)
record("bunny CANNOT edit history (Lion-only)", code == 403, f"HTTP {code} {resp.get('error', '')[:60]}")

code, resp = call("delete", {"node_id": NODE, "from": "bunny", "message_id": "1", "ts": ts, "signature": sign(payload)})
record("bunny CANNOT delete history (Lion-only)", code == 403, f"HTTP {code} {resp.get('error', '')[:60]}")

code, resp = send("tamper target", corrupt=True)
record("tampered body fails signature", code == 403, f"HTTP {code} {resp.get('error', '')[:60]}")

code, resp = send("stale replay", ts=int(time.time() * 1000) - 10 * 60 * 1000)
record("stale timestamp rejected (replay window)", code == 403, f"HTTP {code} {resp.get('error', '')[:60]}")
...
code, resp = call(
    "send", {"node_id": "ghost-node", "from": "bunny", "text": "hello", "ts": ts, "signature": sign(payload)}
)
