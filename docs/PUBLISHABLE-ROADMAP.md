...
- ~~**Hardware QA against v1.2.0**~~ — protocol-level sign-off accepted as sufficient for the v1.2.0 release. `/tmp/v120-qa/drive.py` ran 7 vault round-trips + 9 C1 signature-gate tests green against a throwaway mesh on 2026-04-21; that's the whole crypto surface of the v1.2.0 changes. UI-bound §1a (direct-pair fingerprint pin) + §1b (QR-pair) + §4b (mandatory-reply auto-lock) deliberately deferred to the UI automation track (Medium-term below) rather than blocking on a human-with-rig session — the underlying protocol is verified; what's untested is the UI glue on top of it. v1.2.0 ships as "protocol-verified; UI pairing/messaging regression automated-coverage pending". Pixel 10 rig (serial `57261FDCR004ZQ`) remains available for manual spot-checks if a regression surfaces.
...
- **UI automation for pairing / release flows** *(promoted 2026-04-21 — now the gating item to replace per-release manual QA)*. Three UI flows aren't covered by any automated test today and need a harness if we want hands-off releases:
  - **§1a direct-pair fingerprint pin** (Audit C5 regression) — Lion's Share Setup → Add Bunny → Direct flow. Assertions: correct fp → `PAIRED direct (verified fp=…)`; wrong fp → abort + both-fps shown + no `lion_pubkey` stored on bunny; blank fp → `PAIRED direct (UNVERIFIED fp=…)` status string.
  - **§1b QR-pair** — Lion's Share camera on Bunny Tasker pairing screen. Hardest to automate (inherently camera→screen); realistic path is to bypass the camera by injecting the QR payload via a test-only intent while leaving the verification logic live.
  - **§4b mandatory-reply auto-lock** (Audit C4 regression) — bunny side auto-locks on overdue mandatory reply iff lion signature verifies; unsigned/forged `from:"lion"` must not auto-lock.

  Approach: Appium on a Waydroid/AVD surface (no physical rig dependency); one shared `uv`-managed `tests/ui/` Python module hitting all three APKs via package name. Skeleton + §1a first (highest-value, simplest to drive) — that's the ~1-day pilot. §1b + §4b land in a follow-up once the harness is proven. Success criteria: a regression in any of the three paths breaks CI instead of breaking a release.
...
- ~~**Stable-filename release aliases**~~ — done. `release.yml` now emits unversioned `focuslock-latest.apk` / `bunnytasker-latest.apk` / `focusctl-latest.apk` + `FocusLock-latest.exe` / `FocusLock-Watchdog-latest.exe` copies alongside the tagged artifacts. The tagged copies remain for pinning; both sets are byte-identical within a release and both filenames appear in `SHA256SUMS.txt` / `APK-CERTS.txt`. README Download section updated to link at `releases/latest/download/<name>-latest.<ext>` directly. First release to carry the aliases will be whatever ships after v1.2.0. See CHANGELOG [Unreleased].
