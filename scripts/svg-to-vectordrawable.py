...
def parse_path(d):
    ...
    for cmd, chunk in re.findall(r"([MCZmcz])([^MCZmcz]*)", d):
        ...
        if len(nums) % stride:
            raise ValueError("%s: expected multiples of %d, got %d" % (cmd, stride, len(nums)))
...
def emit(segs, s, tx, ty):
    ...
    for cmd, n in segs:
        if cmd == "Z":
            ...
            continue
        coords = " ".join(
            fmt(v * s + (tx if i % 2 == 0 else ty)) for i, v in enumerate(n)
        )
        parts.append(coords if cmd == last else "%s%s" % (cmd, coords))
        last = cmd
    return " ".join(parts)
...
def read_svg(path):
    ...
    for attrs_src, body in re.findall(r'<linearGradient\b([^>]*)>(?s:(.*?))</linearGradient>', text):
        ...
        stops = [dict(re.findall(r'([\w:-]+)="([^"]*)"', m))
                 for m in re.findall(r"<stop\b([^>]*)/?>", body)]
...
def build(cfg):
    ...
    lines = ['<?xml version="1.0" encoding="utf-8"?>',
             '<!-- Generated by scripts/svg-to-vectordrawable.py from %s. Do not hand-edit. -->'
             % cfg["svg"],
             '<vector xmlns:android="http://schemas.android.com/apk/res/android"']
    if uses_gradient:
        lines.append('    xmlns:aapt="http://schemas.android.com/aapt"')
    lines += ['    android:width="%ddp"' % int(VIEWPORT),
              '    android:height="%ddp"' % int(VIEWPORT),
              '    android:viewportWidth="%d"' % int(VIEWPORT),
              '    android:viewportHeight="%d">' % int(VIEWPORT)]

    for (segs, fill, rule), _ in zip(parsed, boxes):
        ...
        def gpt(xk, yk, dx, dy):
            x = gx0 + pct(gattrs.get(xk, dx)) * gw
            y = gy0 + pct(gattrs.get(yk, dy)) * gh
            return fmt(x * s + tx), fmt(y * s + ty)

        sx, sy = gpt("x1", "y1", "0%", "0%")
        ex, ey = gpt("x2", "y2", "100%", "0%")
        lines.append('    <path%s\n        android:pathData="%s">' % (ftype, data))
        ...
        lines.append('            <gradient\n                android:type="linear"'
                     ...
                     % (sx, sy, ex, ey))
        for st in stops:
            lines.append('                <item android:offset="%s" android:color="%s" />'
    ...
    out.write_text("\n".join(lines) + "\n")
    print("%-11s %s  (scale %.6f, translate %.3f %.3f, %d bytes)"
          % (cfg["out"].split("/")[1], out.relative_to(ROOT), s, tx, ty, out.stat().st_size))
